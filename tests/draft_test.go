...
func TestDraftSavedAgainWithSameID(t *testing.T) {
	// Handle case where remote ID for the new draft is identical
	runOneToOneTestWithAuth(t, defaultServerOptions(t, withConnectorBuilder(&returnSameRemoteIDConnectorBuilder{})), func(c *testConnection, s *testSession) {
		mailboxID := s.mailboxCreatedWithAttributes("user", []string{"Drafts"}, imap.NewFlagSet(imap.AttrDrafts))
...
func testDraftSavedAgainImpl(t *testing.T, builder connectorBuilder) {
	var options []serverOption
	if builder != nil {
		options = append(options, withConnectorBuilder(builder))
	}
