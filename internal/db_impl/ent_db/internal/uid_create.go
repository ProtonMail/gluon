...
func (uc *UIDCreate) Save(ctx context.Context) (*UID, error) {
	uc.defaults()
	return withHooks[*UID, UIDMutation](ctx, uc.sqlSave, uc.mutation, uc.hooks)
}
...
func (uc *UIDCreate) check() error {
	if _, ok := uc.mutation.UID(); !ok {
		return &ValidationError{Name: "UID", err: errors.New(`internal: missing required field "UID.UID"`)}
	}
	if _, ok := uc.mutation.Deleted(); !ok {
		return &ValidationError{Name: "Deleted", err: errors.New(`internal: missing required field "UID.Deleted"`)}
	}
	if _, ok := uc.mutation.Recent(); !ok {
		return &ValidationError{Name: "Recent", err: errors.New(`internal: missing required field "UID.Recent"`)}
	}
	return nil
}
...
func (uc *UIDCreate) createSpec() (*UID, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(uid.Table, sqlgraph.NewFieldSpec(uid.FieldID, field.TypeInt))
	)
