...
func (mu *MailboxUpdate) Save(ctx context.Context) (int, error) {
	return withHooks[int, MailboxMutation](ctx, mu.sqlSave, mu.mutation, mu.hooks)
}
...
func (mu *MailboxUpdate) sqlSave(ctx context.Context) (n int, err error) {
	_spec := sqlgraph.NewUpdateSpec(mailbox.Table, mailbox.Columns, sqlgraph.NewFieldSpec(mailbox.FieldID, field.TypeUint64))
	...
	if mu.mutation.RemoteIDCleared() {
		_spec.ClearField(mailbox.FieldRemoteID, field.TypeString)
	}
	...
	if value, ok := mu.mutation.AddedUIDNext(); ok {
		_spec.AddField(mailbox.FieldUIDNext, field.TypeUint32, value)
	}
	...
	if value, ok := mu.mutation.AddedUIDValidity(); ok {
		_spec.AddField(mailbox.FieldUIDValidity, field.TypeUint32, value)
	}
...
func (muo *MailboxUpdateOne) Save(ctx context.Context) (*Mailbox, error) {
	return withHooks[*Mailbox, MailboxMutation](ctx, muo.sqlSave, muo.mutation, muo.hooks)
}
...
func (muo *MailboxUpdateOne) sqlSave(ctx context.Context) (_node *Mailbox, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`internal: missing "Mailbox.id" for update`)}
	}
