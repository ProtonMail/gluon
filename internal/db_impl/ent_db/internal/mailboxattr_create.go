...
func (mac *MailboxAttrCreate) Save(ctx context.Context) (*MailboxAttr, error) {
	return withHooks[*MailboxAttr, MailboxAttrMutation](ctx, mac.sqlSave, mac.mutation, mac.hooks)
}
...
func (mac *MailboxAttrCreate) check() error {
	if _, ok := mac.mutation.Value(); !ok {
		return &ValidationError{Name: "Value", err: errors.New(`internal: missing required field "MailboxAttr.Value"`)}
	}
	return nil
}
...
func (mac *MailboxAttrCreate) createSpec() (*MailboxAttr, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(mailboxattr.Table, sqlgraph.NewFieldSpec(mailboxattr.FieldID, field.TypeInt))
	)
