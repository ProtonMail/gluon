...
func (uu *UIDUpdate) Save(ctx context.Context) (int, error) {
	return withHooks[int, UIDMutation](ctx, uu.sqlSave, uu.mutation, uu.hooks)
}
...
func (uu *UIDUpdate) sqlSave(ctx context.Context) (n int, err error) {
	_spec := sqlgraph.NewUpdateSpec(uid.Table, uid.Columns, sqlgraph.NewFieldSpec(uid.FieldID, field.TypeInt))
	...
	if value, ok := uu.mutation.UID(); ok {
		_spec.SetField(uid.FieldUID, field.TypeUint32, value)
	}
	if value, ok := uu.mutation.AddedUID(); ok {
		_spec.AddField(uid.FieldUID, field.TypeUint32, value)
	}
	if value, ok := uu.mutation.Deleted(); ok {
		_spec.SetField(uid.FieldDeleted, field.TypeBool, value)
	}
	if value, ok := uu.mutation.Recent(); ok {
		_spec.SetField(uid.FieldRecent, field.TypeBool, value)
	}
...
// Where appends a list predicates to the UIDUpdate builder.
func (uuo *UIDUpdateOne) Where(ps ...predicate.UID) *UIDUpdateOne {
...
func (uuo *UIDUpdateOne) Save(ctx context.Context) (*UID, error) {
	return withHooks[*UID, UIDMutation](ctx, uuo.sqlSave, uuo.mutation, uuo.hooks)
}
...
func (uuo *UIDUpdateOne) sqlSave(ctx context.Context) (_node *UID, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`internal: missing "UID.id" for update`)}
	}
