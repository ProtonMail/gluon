...
func (mc *MailboxCreate) Save(ctx context.Context) (*Mailbox, error) {
	mc.defaults()
	return withHooks[*Mailbox, MailboxMutation](ctx, mc.sqlSave, mc.mutation, mc.hooks)
}
...
func (mc *MailboxCreate) check() error {
	if _, ok := mc.mutation.Name(); !ok {
		return &ValidationError{Name: "Name", err: errors.New(`internal: missing required field "Mailbox.Name"`)}
	}
	if _, ok := mc.mutation.UIDNext(); !ok {
		return &ValidationError{Name: "UIDNext", err: errors.New(`internal: missing required field "Mailbox.UIDNext"`)}
	}
	if _, ok := mc.mutation.UIDValidity(); !ok {
		return &ValidationError{Name: "UIDValidity", err: errors.New(`internal: missing required field "Mailbox.UIDValidity"`)}
	}
	if _, ok := mc.mutation.Subscribed(); !ok {
		return &ValidationError{Name: "Subscribed", err: errors.New(`internal: missing required field "Mailbox.Subscribed"`)}
	}
	return nil
}
...
func (mc *MailboxCreate) createSpec() (*Mailbox, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(mailbox.Table, sqlgraph.NewFieldSpec(mailbox.FieldID, field.TypeUint64))
	)
	...
	if value, ok := mc.mutation.RemoteID(); ok {
		_spec.SetField(mailbox.FieldRemoteID, field.TypeString, value)
	...
	if value, ok := mc.mutation.UIDNext(); ok {
		_spec.SetField(mailbox.FieldUIDNext, field.TypeUint32, value)
		_node.UIDNext = value
	}
	if value, ok := mc.mutation.UIDValidity(); ok {
		_spec.SetField(mailbox.FieldUIDValidity, field.TypeUint32, value)
		_node.UIDValidity = value
	}
	if value, ok := mc.mutation.Subscribed(); ok {
		_spec.SetField(mailbox.FieldSubscribed, field.TypeBool, value)
