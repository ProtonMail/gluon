...
func (user *user) apply(ctx context.Context, update imap.Update) error {
	...
	err := userDBWrite(ctx, user, func(ctx context.Context, transaction db.Transaction) ([]state.Update, error) {
		updates, err := user.applyUpdateTx(ctx, transaction, &storeCleanup, update)
		if err != nil {
			return nil, err
		}
	...
	if err != nil {
		if err := storeCleanup.clean(user.store); err != nil {
			logrus.WithError(err).Error("Failed to cleanup store data after failed transaction")
		}
	}
...
type storeCleanup struct {
	ids []imap.InternalMessageID
}

func (c *storeCleanup) add(id imap.InternalMessageID) {
	c.ids = append(c.ids, id)
}

func (c *storeCleanup) clean(store store.Store) error {
	if len(c.ids) == 0 {
		return nil
	}
	...
	for _, id := range c.ids {
		if err := store.Delete(id); err != nil {
			if !os.IsNotExist(err) {
				logrus.WithError(err).Errorf("Failed to delete cache message %v after failed transaction", id)
			}
		}
	}
...
func (user *user) applyUpdateTx(ctx context.Context, tx db.Transaction, sc *storeCleanup, update imap.Update) ([]state.Update, error) {
	switch update := update.(type) {
	case *imap.BatchUpdate:
		stateUpdates := make([]state.Update, 0, len(update.Updates))

		for idx, u := range update.Updates {
			up, err := user.applyUpdateTx(ctx, tx, sc, u)
			if err != nil {
				return nil, fmt.Errorf("failed to apply batch update %v: %w", idx, err)
			}

			stateUpdates = append(stateUpdates, up...)
		}
	...
	case *imap.MessagesCreated:
		return user.applyMessagesCreated(ctx, tx, sc, update)

	case *imap.MessageMailboxesUpdated:
		return user.applyMessageMailboxesUpdated(ctx, tx, update)
	...
	case *imap.MessageUpdated:
		return user.applyMessageUpdated(ctx, tx, sc, update)

	case *imap.UIDValidityBumped:
		return user.applyUIDValidityBumped(ctx, tx)

	case *imap.Noop:
		return nil, nil

	default:
		return nil, fmt.Errorf("bad update")
	}
}

// applyMailboxCreated applies a MailboxCreated update.
func (user *user) applyMailboxCreated(ctx context.Context, tx db.Transaction, update *imap.MailboxCreated) ([]state.Update, error) {
	if update.Mailbox.ID == ids.GluonInternalRecoveryMailboxRemoteID {
		return nil, fmt.Errorf("attempting to create protected mailbox (recovery)")
	}

	if exists, err := tx.MailboxExistsWithRemoteID(ctx, update.Mailbox.ID); err != nil {
		return nil, err
	} else if exists {
		return nil, nil
	}
...
func (user *user) applyMailboxDeleted(ctx context.Context, tx db.Transaction, update *imap.MailboxDeleted) ([]state.Update, error) {
	if update.MailboxID == ids.GluonInternalRecoveryMailboxRemoteID {
		return nil, fmt.Errorf("attempting to delete protected mailbox (recovery)")
	}
...
func (user *user) applyMailboxUpdated(ctx context.Context, tx db.Transaction, update *imap.MailboxUpdated) ([]state.Update, error) {
	if update.MailboxID == ids.GluonInternalRecoveryMailboxRemoteID {
		return nil, fmt.Errorf("attempting to rename protected mailbox (recovery)")
	}
	...
	return nil, tx.RenameMailboxWithRemoteID(ctx, update.MailboxID, strings.Join(update.MailboxName, user.delimiter))
}

// applyMailboxIDChanged applies a MailboxIDChanged update.
func (user *user) applyMailboxIDChanged(ctx context.Context, tx db.Transaction, update *imap.MailboxIDChanged) ([]state.Update, error) {
	if update.InternalID == user.recoveryMailboxID {
		return nil, fmt.Errorf("attempting to change protected mailbox (recovery) remote ID")
	}
...
func (user *user) applyMessagesCreated(ctx context.Context, tx db.Transaction, sc *storeCleanup, update *imap.MessagesCreated) ([]state.Update, error) {
...
func (user *user) applyMessageMailboxesUpdated(ctx context.Context, tx db.Transaction, update *imap.MessageMailboxesUpdated) ([]state.Update, error) {
	if slices.Contains(update.MailboxIDs, ids.GluonInternalRecoveryMailboxRemoteID) {
		return nil, fmt.Errorf("attempting to move messages into protected mailbox (recovery)")
	}

	if exists, err := tx.MessageExistsWithRemoteID(ctx, update.MessageID); err != nil {
		return nil, err
	} else if !exists {
...
func (user *user) applyMessageFlagsUpdated(ctx context.Context, tx db.Transaction, update *imap.MessageFlagsUpdated) ([]state.Update, error) {
	if exists, err := tx.MessageExistsWithRemoteID(ctx, update.MessageID); err != nil {
		return nil, err
	} else if !exists {
		return nil, state.ErrNoSuchMessage
...
func (user *user) applyMessageIDChanged(ctx context.Context, tx db.Transaction, update *imap.MessageIDChanged) ([]state.Update, error) {
	if err := tx.UpdateRemoteMessageID(ctx, update.InternalID, update.RemoteID); err != nil {
		return nil, err
	}

	return []state.Update{state.NewMessageRemoteIDUpdateStateUpdate(update.InternalID, update.RemoteID)}, nil
}
...
func (user *user) applyMessageDeleted(ctx context.Context, tx db.Transaction, update *imap.MessageDeleted) ([]state.Update, error) {
...
func (user *user) applyMessageUpdated(ctx context.Context, tx db.Transaction, sc *storeCleanup, update *imap.MessageUpdated) ([]state.Update, error) {
	log := logrus.WithField("message updated", update.Message.ID.ShortID())

	internalMessageID, err := tx.GetMessageIDFromRemoteID(ctx, update.Message.ID)
	if db.IsErrNotFound(err) {
		if update.AllowCreate {
			log.Warn("Message not found, creating it instead")

			return user.applyMessagesCreated(ctx, tx, sc, imap.NewMessagesCreated(true, &imap.MessageCreated{
...
func (user *user) applyUIDValidityBumped(ctx context.Context, tx db.Transaction) ([]state.Update, error) {
	...
	return []state.Update{state.NewUIDValidityBumpedStateUpdate()}, nil
}
